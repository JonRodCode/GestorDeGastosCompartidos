  ...
  const fraseDeEliminacion =
      ...
      nuevosConsumos.forEach((consumo) => {
        if (pendientes.includes(consumo)) {
          ...
          if (existenteEn) {
            ...
            const fuenteNueva = Object.entries(nuevas.fuenteDelGasto).find(([_, lista]) =>
    ...
    const verificarSiHayDatos = () => {
      ...
      return Object.entries(especificaciones).some(([key, value]) => {
        if (key === "determinaciones" && typeof value === "object" && value !== null) {
    ...
    const agregarNuevasEspecificaciones = (nuevas) => {
      ...
      Object.entries(especificaciones.determinaciones).forEach(([key, values]) => {
...
{mostrarInputEspecificaciones && (
              <PanelDeCargaDeEspecificaciones
                ...
                verificarSiHayConflictosConLosElementosEnUso={verificarSiHayConflictosConLosElementosEnUso}
              />
            )}
        </div>
